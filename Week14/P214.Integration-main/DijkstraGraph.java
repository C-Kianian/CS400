...
public class DijkstraGraph<NodeType, EdgeType extends Number> extends BaseGraph<NodeType, EdgeType>
  ...
  protected SearchNode computeShortestPath(NodeType start, NodeType end) {
    ...
    HashtableMap<Node, Double> visitedSet = new HashtableMap<>();//visited nodes + cost
    ...
    while (!queue.isEmpty()) {
      ...
      if (visitedSet.containsKey(curr.node) && (double)visitedSet.get(curr.node) <= curr.cost) {
        //check if node has been visited and is smaller than its previous cost
        continue;
      }
      ...
      if (curr.node.data.equals(end))
        return curr;//reached the end node

      for (Edge edge : curr.node.edgesLeaving) {//for all edges of removed node see if new/better path
        double newCost = curr.cost + edge.data.doubleValue();//new cost to reach
        if (!visitedSet.containsKey(edge.successor) || newCost < (double)visitedSet.get(edge.successor)) {
          //if not already visited add, otherwise ensure it path is shorter than previous
          queue.add(new SearchNode(edge.successor, newCost, curr));
        }
      }
    }
